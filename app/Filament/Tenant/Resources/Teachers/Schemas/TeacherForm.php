...
namespace App\Filament\Tenant\Resources\Teachers\Schemas;
    ...
    public static function configure(Schema $schema): Schema
    {
        ...
        ->components([
            ...
            TextInput::make('email')
                ->email()
                ->maxLength(255)
                ->rule(fn (?Teacher $record) => Rule::unique('teachers', 'email')
            ...
            TextInput::make('employee_code')
                ->label('Employee Code')
                ->maxLength(50)
                ->rule(fn (?Teacher $record) => Rule::unique('teachers', 'employee_code')
                    ->where('tenant_id', tenant('id'))
                    ->ignore($record?->getKey())
                ),
